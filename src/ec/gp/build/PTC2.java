 ...
 *
 * <p> Generally speaking, PTC2 picks a random position in the horizon of the tree (unfiled child node positions), fills it with a nonterminal, thus extending the horizon, and repeats this until the number of nodes (nonterminals) in the tree, plus the number of unfilled node positions, is >= the requested tree size.  Then the remaining horizon is filled with terminal.
 *
 ...
 *
 * <p> PTC2 also has provisions for picking nonterminals with a certain probability over other nonterminals of the same return type (and terminal over other terminal likewise), hence its name.  To change the probability of picking various terminal or nonterminals, you modify your PTCFunctionSetForm function set.
 *
 * <p>PTC2 further has a maximum depth, which you should set to some fairly big value.  If your maximum depth is small enough that PTC2 often creates trees which bump up against it, then PTC2 will only generate terminal at that depth position.  If the depth is *really* small, it's possible that this means PTC2 will generate trees smaller than you had requested.
 *
 <p><b>Parameters</b><br>
 <table>
...
public class PTC2 extends GPNodeBuilder 
    ...
    public GPNode newRootedTree(final EvolutionState state,
        ...
        int requestedSize)
        {
        // ptc2 can mess up if there are no available terminal for a given type.  If this occurs,
