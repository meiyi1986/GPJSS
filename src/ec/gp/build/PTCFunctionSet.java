...
public class PTCFunctionSet extends GPFunctionSet implements PTCFunctionSetForm
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        if (allZeros)
            state.output.warning("In this function set, the probabilities of all nonterminal function have a 0.0 selection probability -- this will cause them all to be selected uniformly.  That could be an error.", base);
        allZeros = false;
        ...
        if (allZeros)
            state.output.warning("In this function set, the probabilities of all terminal function have a 0.0 selection probability -- this will cause them all to be selected uniformly.  That could be an error.", base);

        if (!allOnes && !noOnes)
            state.output.warning("In this function set, there are some function with a selection probability of 1.0, but not all of them.  That could be an error.",base);
        
        // set up our node probabilities.  Allow all zeros.
        for(int x=0;x<q_ty.length;x++)
            {
            if (q_ty[x].length == 0) state.output.warning("Function Set " + name + " has no terminal for type number " + x + ".  This may cause problems for you.");
