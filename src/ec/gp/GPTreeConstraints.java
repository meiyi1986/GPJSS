...
public class GPTreeConstraints implements Clique
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        while (e.hasMoreElements())
            ...
            if (i.length==0) // yeesh
                state.output.error("In function set " + functionset + " for the GPTreeConstraints " + this + ", no nodes at all are given with the return type " + t + " which is required by other function in the function set or by the tree's return type.  This almost certainly indicates a serious typing error.", base);
            else
                {
                i = functionset.terminals[t.type];
                if (i.length==0) // uh oh
                    state.output.warning("In function set " + functionset + " for the GPTreeConstraints " + this + ", no terminal are given with the return type " + t + " which is required by other function in the function set or by the tree's return type.  Nearly all tree-builders in ECJ require the ability to add a terminal of any type for which there is a nonterminal, and at any time.  Without terminal, your code may not work.  One common indication that a tree-builder has failed due to this problem is if you get the MersenneTwister error 'n must be positive'.", base);
                i = functionset.nonterminals[t.type];
                if (i.length==0) // uh oh
                    state.output.warning("In function set " + functionset + " for the GPTreeConstraints " + this + ", no *nonterminals* are given with the return type " + t + " which is required by other function in the function set or by the tree's return type.  This may or may not be a problem for you.", base);
                }
            }
